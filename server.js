...
function recipeResponse(recipe, username) {
    ...
    recipeObject.ratingCount = recipe.ratings ? recipe.ratings.length : 0;
...
async function deliverOtpEmail(email, otp) {
    const message = `Your Apna Kitchen password reset OTP is ${otp}. It expires in 10 minutes.`;

    if (!process.env.SMTP_HOST) {
        console.log(`[DEV OTP] ${email}: ${message}`);
        return { delivered: false, message };
    }

    console.log(`[OTP EMAIL READY] Configure SMTP delivery for ${email}: ${message}`);
...
app.post('/forgot-password/request-otp', async (req, res) => {
    try {
        ...
        const delivery = await deliverOtpEmail(user.email, otp);
        res.json({
            success: true,
            message: delivery.delivered
                ? 'OTP sent to your email.'
                : 'OTP generated. Check the server console in development mode.',
